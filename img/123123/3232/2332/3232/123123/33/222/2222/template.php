				...
				<div class="flexbox flexbox--row flex-wrap align-items-normal <?=!$bShowPropsBlock ? 'justify-center' : ''?>">
					<?if($bShowPropsBlock):?>
						<div class="product-chars flex-50">
							...
							<?if( ($arResult['DISPLAY_PROPERTIES'] || $arResult['OFFER_PROP']) && $arParams['VISIBLE_PROP_COUNT'] > 0 ):?>
								<div class="char-side">
									<div class="char-side__title darken"><?=($arParams["T_CHARACTERISTICS"] ? $arParams["T_CHARACTERISTICS"] : Loc::getMessage("T_CHARACTERISTICS"));?></div>
									<div class="properties list _uprops">
										<div class="_list properties <?=!$bShowMoreLink ? 'js-offers-prop' : ''?>">
											<?$j=0;?>
											<?foreach($arResult['DISPLAY_PROPERTIES'] as $arProp):?>
												<?if($j<$arParams['VISIBLE_PROP_COUNT']):?>
													<div class="_item">
														<div class="_label">
                              <div class="_label__val"><?=$arProp['NAME']?></div>
					...
					<div class="product-action flex-50">
						...
						<div class="info_item">
											...
											<div class="cost prices detail">
												...
												<?else:?>
													...
													<?elseif(isset($arResult["PRICES"])):?>
														<?//проверка цена = цена по акции | demis ?>
														<?if(count($arResult["PRICES"]) > 1):?>
															<?if($arResult["PRICES"]["Соглашение для ИМ"]["VALUE_VAT"] == $arResult["PRICES"]["Соглашение для Маркетинг"]["VALUE_VAT"]):?>
																<?unset($arResult["PRICES"]["Соглашение для Маркетинг"]);?>
															<?endif;?>
														<?endif;?>
							...
							<?if(!empty($arResult["PROPERTIES"]["TEXT_DISCLAIMER"]) && !empty($arResult["PROPERTIES"]["TEXT_DISCLAIMER"]["VALUE"]["TEXT"])):?>
								<div class="text-form">
									<div class="price_txt muted777 font_sxs muted ncolor">
											<?=$arResult["PROPERTIES"]["TEXT_DISCLAIMER"]["VALUE"]["TEXT"];?>
									</div>
								</div>
							<?endif;?>
		...
		<div class="hidden_print">
			<div class="video_block row">
				...
				<?else:?>
					<div class="col-md-12">
						<?if($arResult["ID"] == 1222):?>
							<div class="ordered-block__title option-font-bold font_lg 1">Видео изготовления прозрачного ключа для долговременного композитного мокапа с помощью материала для регистрации прикуса Bisico Regi-trans.</div>							
						<?elseif($arResult["ID"] == 1334):?>
							<div class="ordered-block__title option-font-bold font_lg 1">Процесс оформления краев индивидуальной ложки для получения функционального оттиска при полном съемном протезировании с использованием Bisico Function.</div>
						<?endif;?>
						<?=$arResult['VIDEO'][0]?>
					</div>
				<?endif;?>
...
<?if($bItemFiles):?>
	...
	$templateData['DOCS'] = $arFiles;
	if($arFiles):?>
		<?$this->SetViewTarget('PRODUCT_DOCS_INFO');?>
			<div class="char_block rounded3 bordered files_block">
				<div class="row flexbox">
					<?foreach($arFiles as $arItem):?>
						<div class="col-md-4 col-sm-6 col-xs-12">
...
<?//lisencec?>
<?$instr_prop1 = "LICENSES";?>
<?
$bItemFiles1 = isset($arResult["PROPERTIES"][$instr_prop1]) && (is_array($arResult["PROPERTIES"][$instr_prop1]["VALUE"]) && count($arResult["PROPERTIES"][$instr_prop1]["VALUE"]));
?>
<?if($bItemFiles1):?>
	<?
	$arFiles = array();
	if($arResult["PROPERTIES"][$instr_prop1]["VALUE"])
	{
		$arFiles = $arResult["PROPERTIES"][$instr_prop1]["VALUE"];
	}
	if(is_array($arFiles))
	{
		foreach($arFiles as $key => $value)
	...
	$templateData['LICENSES'] = $arFiles;
	if($arFiles):?>
		<?$this->SetViewTarget('PRODUCT_LICENSES_INFO');?>
			<div class="char_block rounded3 bordered files_block">
				<div class="row flexbox">
					<?foreach($arFiles as $arItem):?>
						<div class="col-md-4 col-sm-6 col-xs-12">
