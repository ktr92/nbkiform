...
<div class="slam-easyform<?=$arParams['HIDE_FORMVALIDATION_TEXT'] == 'Y' ? ' hide-formvalidation' : ''?>" <?=$WITH_FORM?>>
    <form id="<?=$FORM_ID?>"
          ...
          data-default="<?=$arParams['FORM_SUBMIT_VALUE']?>"
            ...
            if(!empty($arResult['FORM_FIELDS'])):
                foreach($arResult['FORM_FIELDS'] as $fieldCode => $arField):
                    ...
                    <?elseif($arField['TYPE'] == 'file'):?>
                        <div class="col-xs-12">
                            <div class="form-group">
                                <? if(!$arParams['HIDE_FIELD_NAME'] && 1==2): ?>
                                    <label class="control-label" for="<?=$arField['ID']?>"><?=$arField['TITLE']?></label>
                                <? endif; ?>
                ...
                <div class="col-xs-12">
                    <div class="form-group form-group-captcha">
                        <input id="<?=$FORM_ID?>-captchaValidator"  class="form-control" type="text" required data-bv-notempty-message="<?=GetMessage("SLAM_REQUIRED_MESS");?>" value="" name="captchaValidator" style="border: none; height: 0; padding: 0; visibility: hidden;">
                    </div>
                </div>
            <?endif;?>
                ...
                <div class="col-xs-12">
                    <?if($arResult['WARNING_MSG']):?>
                        ...
                        <div class="is-form-field field-group is-form__group _field-inited state-inited" >
                            <div class="is-form-field field-checkbox">
                                <label>
                                    <input type="checkbox" name="agreement"><i></i>
                                    <span class="checkbox-label">Я ознакомлен и согласен <a class="is-link" href="/privacy/" target="_blank">«C&nbsp;условиями сбора&nbsp;и&nbsp;обработки персональных данных»</a>.</span>
                                </label>
                            </div>
                        </div>
                    <?endif;?>
                    <button type="submit" disabled='disabled' class="wpcf7-form-control wpcf7-submit submit-button" data-metrika="otpravit" ><?=$arParams['FORM_SUBMIT_VALUE']?></button>
                </div>
            <?endif;?>
        </div>
    </form>
    ...
    $(document).ready(function() {
        function checkAgree() {
            if ($('[name="agreement"]').is(':checked')) {
                $('[name="agreement"]').closest('form').find('[type="submit"]').removeAttr('disabled')
            } else {
                $('[name="agreement"]').closest('form').find('[type="submit"]').attr('disabled', 'disabled')
            }
        }
