...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.steptotalInfoBlockNode = BX('empis__ordertotal');
...
/* 
		...
		getBlockFooter: function(node)
		{
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('button', {
						props: {href: 'javascript:void(0)', className: 'pull-right btn btn-default pl-3 pr-3'},
		...
		/* 	if (this.result.SHOW_AUTH && section.id != this.authBlockNode.id && section.id != this.basketBlockNode.id)
			...
			var active = true,
			// для блока доставки делаем отдельный флаг, т.к. иначе блок ломается при авторизации
			 activeDelivery = this.result.IS_AUTHORIZED ? section.id == this.activeSectionId : true,
				titleNode = section.querySelector('.bx-soa-section-title-container'),
		...
		/* 	BX.unbindAll(titleNode);
			...
			switch (section.id)
			{
				...
				case this.paySystemBlockNode.id:
					...
					break;
				case this.deliveryBlockNode.id:
					this.editDeliveryBlock(activeDelivery);
					break;
				case this.pickUpBlockNode.id:
							...
							children: [
								BX.create('INPUT', {
									props: {
										...
										type: 'text',
										placeholder: BX.message("COUPON_DEFAULT")
									},
									events: {
		...
		createDeliveryItem: function(item)
		{
			...
			labelNodes.push(
				BX.create('DIV', {
					props: {className: 'bx-soa-pp-wrapperblock'},
					children: [title, pp_logo]
				})
		...
		editFadeDeliveryContent: function(node)
		{
			...
			// здесь уже выбрана доставка по умолчанию
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.IS_PAYER == 'Y') {
					...
					textNode.closest('.form-group').classList.add('empisbasket__input_full')
				}
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				inputs[i].closest('.form-group').classList.add('empisbasket__input_full')
			}
		...
		editPropsComment: function(propsNode)
		{
			...
			div = BX.create('DIV', {
				props: {className: 'form-group bx-soa-customer-field empisbasket__input_full'},
		...
		editStepTotalBlock: function()
		{
			if (!this.steptotalInfoBlockNode || !this.result.TOTAL)
				return;
			...
			BX.cleanNode(this.steptotalInfoBlockNode);
			...
			this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params));

			if (this.options.showOrderWeight)
			{
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM'), total.ORDER_WEIGHT_FORMATED));
			}
			...
			if (this.options.showTaxList)
			{
				...
				{
					valFormatted = total.TAX_LIST[i].VALUE_MONEY_FORMATED || '';
					this.steptotalInfoBlockNode.appendChild(
						this.createTotalUnit(
							total.TAX_LIST[i].NAME + (!!total.TAX_LIST[i].VALUE_FORMATED ? ' ' + total.TAX_LIST[i].VALUE_FORMATED : '') + ':',
							valFormatted
						)
			...
			if (this.result.DELIVERY.length)
			{
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params));
			}
			...
			if (this.options.showDiscountPrice)
			{
				...
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(discText + ':', total.DISCOUNT_PRICE_FORMATED, {highlighted: true}));
			}

			if (this.options.showPayedFromInnerBudget)
			{
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED));
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_PAYED'), total.PAYED_FROM_ACCOUNT_FORMATED));
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_LEFT_TO_PAY'), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {total: true}));
			}
			else
			{
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, {total: true}));
			}

			if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length)
			{
				this.steptotalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_PAYSYSTEM_PRICE'), '~' + total.PAY_SYSTEM_PRICE_FORMATTED));
			}
