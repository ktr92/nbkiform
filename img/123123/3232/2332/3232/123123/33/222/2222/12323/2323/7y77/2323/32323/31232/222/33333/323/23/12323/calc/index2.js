$(document).ready(function(){
    ...
    function sendoptions()  {
        let result = ""
        if ($('[name=extra_warm]').is(':checked')) {
            result += '&TEPLO=' + $('[name=extra_warm]').val()
        }
        if ($('[name=shipment_intake]').is(':checked')) {
            result += '&send_address=' + $('[name=shipment_intake]').val()
        }
        if ($('[name=received_intake]').is(':checked')) {
            result += '&receive_address=' + $('[name=received_intake]').val()
        }
        if ($('[name=lathing]').is(':checked')) {
            result += '&CRATE=' + $('[name=lathing]').val()
        }
    ...
    function setParams() {
        const params = 'from_text=' + $('[name="from_text"]').val() + '&from=' + $('[name="shipment_city_id"]').val() + '&to_text=' + $('[name="to_text"]').val() + '&to=' + $('[name="received_city_id"]').val() + '&weight=' + $('[name="weight"]').val() + '&volume=' + $('[name="volume"]').val() + '&volume=' + $('[name="volume"]').val() 
        + sendoptions() + '&delivery-type=' + $('[name=delivery-type]:checked').val();

        return params
    }

    function makeCode(params) {
        const $iframe = window.parent.$('.copytextarea')
        const value = $iframe.val()
        let result = value
        if (params.length) {
            result = `<iframe id="azlogCalc" src="http://62.109.31.113/calc/iframe.php?${params}" style="border: none; width: 500px; height: 900px" seamless></iframe>`
        }
        $iframe.val(result)
        return result
    }
    ...
    function changeURLparam(param, value) {
        let href = new URL($('.checkout').attr('href'))
    ...
    $(document).on(
        ...
        function (e) {
            ...
            $('.checkout').attr('href', changeURLparam('delivery-type', $(this).val()))
            makeCode(params)
        }
      )
    $(document).on(
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.success == 1){
                    ...
                    $('.calcresults__item:visible:first input[type="radio"]').trigger('change')
                    checkRadio($('.calcresults__item input[type="radio"]:checked'))

                  
                    
                    const params = setParams()
                    makeCode(params)
                    $('.checkout').attr('href', 'https://azlog.ru/calc/?' + params);
                  

/*                     $('.checkout').attr('href', changeURLparam('delivery-type', $('[name="delivery-type"]').val()))
 */
                }else{
