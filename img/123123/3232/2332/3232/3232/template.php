...
<?if($arResult["ITEMS"]):?>
	...
	<? /* $count = 0; */ ?>
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$dataItem = ($bOrderViewBasket ? CAllcorp3Resort::getDataItem($arItem) : false);
			...
			if ($arCurrentOffer) {
				...
				$arItem['OFFER_PROP'] = CAllcorp3Resort::PrepareItemProps($arCurrentOffer['DISPLAY_PROPERTIES']);
			...
			<?ob_start();?>
				<?if ($arParams["SHOW_DISCOUNT_TIME"]=="Y" && $arParams['SHOW_COUNTER_LIST'] != 'N' && $arItem['DISPLAY_PROPERTIES']['DATE_COUNTER']['VALUE']):?>
					<?\Aspro\Functions\CAsproAllcorp3Resort::showDiscountCounter([
						'TYPE' => 'list-image',
						'ICONS' => true,
			...
			<div class="catalog-block__wrapper grid-list__item grid-list-border-outer <?=($bSlider ? 'height-100' : '')?> <?=($arCurrentOffer ? 'has-offers' : '');?>" data-hovered="false">
				<div class="catalog-block__item <?=$itemClass?>" id="<?=$arItem["strMainID"]?>">
					<div class="catalog-block__inner flexbox height-100" <?if ($bUseSchema):?>itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product"<?endif;?>>
						...
						<?endif;?>
							<?php
							// Добавляем GET-параметры к ссылке на изображение
							if (isset($_GET['datain'])) {
								$arItem['DETAIL_PAGE_URL'] .= '?datain=' . $_GET['datain'] . '&dataout=' . $_GET['dataout'] . '&adults=' . $_GET['adults'] . '&child=' . $_GET['child'];
							}
							?>
							
							<?=\Aspro\Functions\CAsproAllcorp3Resort::showImage(
								array_merge(
									[
										'ITEM' => $arItem,
										'PARAMS' => $arParams,
						...
						>
							<div class="catalog-block__info-top">
								...
								<div class="catalog-block__info-inner">
									...
									if (0):
									?><div class="catalogparams">
										<div class="catalogparams__items">
										<?if (isset($arItem['PROPERTIES']['PLACE']['VALUE'])):?>
											<div class="catalogparams__item">
											<?=$arItem['PROPERTIES']['PLACE']['VALUE']; ?> м²
											</div>
											<?endif;?>
											<?if (isset($arItem['PROPERTIES']['NUMBER_OF_BEDS']['VALUE'])):?>
											<div class="catalogparams__item">
												<?=$arItem['PROPERTIES']['NUMBER_OF_BEDS']['VALUE']; ?> кровати
											</div>
											<?endif;?>
											<?if (isset($arItem['PROPERTIES']['INCLUDE']['VALUE'])):?>
											<div class="catalogparams__item">
												<?=$arItem['PROPERTIES']['INCLUDE']['VALUE']; ?> гостей
											</div>
											<?endif;?>
											<?if (isset($arItem['PROPERTIES']['SLEEP_ROOMS']['VALUE'])):?>
											<div class="catalogparams__item">
												<?=$arItem['PROPERTIES']['SLEEP_ROOMS']['VALUE']; ?> спален
											</div>
											<?endif;?>
										</div>
									</div>
									...
									if (0):
									?>
									<div class="catalogpricesblock">
										<div class="catalogpricesblock__old">19 990 ₽ </div>
										<div class="catalogpricesblock__current">14 990 ₽ </div>
										<div class="catalogpricesblock__text">за N суток</div>
									</div>
									<a href="#" class="catalogpricesutochno">
										<div class="catalogpricesutochno__value">-5000 ₽ </div>
										<div class="catalogpricesutochno__text">от <b>суточно.ру</b> </div>
										<div class="catalogpricesutochno__info"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50">
										<path fill=" #0000ff" d="M 25 2 C 12.309295 2 2 12.309295 2 25 C 2 37.690705 12.309295 48 25 48 C 37.690705 48 48 37.690705 48 25 C 48 12.309295 37.690705 2 25 2 z M 25 4 C 36.609824 4 46 13.390176 46 25 C 46 36.609824 36.609824 46 25 46 C 13.390176 46 4 36.609824 4 25 C 4 13.390176 13.390176 4 25 4 z M 25 11 A 3 3 0 0 0 22 14 A 3 3 0 0 0 25 17 A 3 3 0 0 0 28 14 A 3 3 0 0 0 25 11 z M 21 21 L 21 23 L 22 23 L 23 23 L 23 36 L 22 36 L 21 36 L 21 38 L 22 38 L 23 38 L 27 38 L 28 38 L 29 38 L 29 36 L 28 36 L 27 36 L 27 21 L 26 21 L 22 21 L 21 21 z"></path>
										</svg></div>
									</a>
							...
							<div class="catalog-block__info-bottom">
								<div class="line-block line-block--20 flexbox--wrap flexbox--justify-beetwen <?=(!$bBottomButtons ? '' : 'catalog-block--bottom-btn');?>">
									...
									<?if (isset($arItem['PROPERTIES']['PRICE_SUT_SRED']) && $arItem['PROPERTIES']['PRICE_SUT_SRED']['VALUE'] > 0) {?>
										<div  class="line-block__item catalog-block__info-bottom--margined js-popup-price <?=(!$bBottomButtons ? 'catalog-block__info-price' : '');?>">
											<?/*=\Aspro\Functions\CAsproAllcorp3Resort::showPrice([
                                    ...
                                    if(!empty($datain)) {
                                        ...
                                        if (!empty($arItem['SUTOCHNO'])) {

                                            $dd = Hotels::priceperiodlist($datain, $dataout, $arItem['ID']);


                                            echo '<span>' . $arItem['SUTOCHNO'] . 'руб. за ' . $dd['total_nights'] . ' дней' . '</span>';


                                        } else {
                                            $dd = Hotels::priceperiodlist($datain, $dataout, $arItem['ID']);
                                            echo '<span>' . $arItem['PRICESITE'] . 'руб. за ' . $dd['total_nights'] . ' дней' . '</span>';
                                        }
                                    } else {


                                        echo '<span>' . $arItem['PROPERTIES']['PRICE_SUT_SRED']['VALUE'] . 'руб. за 1 день' . '</span>';
									...
									<div class="line-block__item catalog-block__info-bottom--margined <?=(!$bBottomButtons ? 'catalog-block__info-btn' : '');?> <?=($arCurrentOffer ? 'hide-600' : '');?>">
										<div class="js-replace-btns js-config-btns" data-btn-config='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arBtnConfig, false, true))?>'>
                                            <?if(!empty($datain)):?>
											<?=\Aspro\Functions\CAsproAllcorp3Resort::showBasketButton(
												array_merge(
													$arBtnConfig,
													[
														'ITEM' => ($arCurrentOffer ? $arCurrentOffer : $arItem),
							...
							<div class="catalog-block__offers hide-600">
								...
								>
									<div class="line-block line-block--flex-wrap line-block--40 line-block--align-normal">
										<?=\Aspro\Allcorp3Resort\Functions\CSKUTemplate::showSkuPropsHtml($arItem['SKU']['PROPS'])?>
									</div>
								</div>
							</div>
						<?endif;?>
