function smzBanner() {
  if ($("#smzbanner") && $("#smzbanner").length) {
    let formSmzToTop = $("#smzbanner").offset().top
    $("#smzbanner").addClass("fixed")
    $("#smzbanner img").css("top", formSmzToTop)
  }
}


$(document).ready(function () {
 $('.notallowed').on('click', function(e) {
    e.preventDefault();
  });

  $('[data-allow="payment"]').each(function() {
    const $wrapper = $(this);
    const $check = $wrapper.find('input[type="checkbox"]');
    const $button = $wrapper.find('[data-btnallow]');
...
/*   if (!getCookie('twentybanner')) {
    $('.twentybanner').show()
   $('.headermain').addClass('headermain_hasbanner')
    $('.twentybanner_mobile').show()
    $('.twentylink').hide()
  } else {
    $('.twentybanner_mobile').hide()
    $('.twentylink').show()
  } */

  $('.twentybanner__button a').on('click', function(e) {
  /*   $('.twentybanner').hide() */
    let date = new Date(Date.now() + 86400e3 * 5);
     date = date.toUTCString();
    document.cookie = "twentybanner=true; expires=" + date; 
  })

  $('[data-action="hidebanner"]').on('click', function(e) {
    $('.twentybanner').slideUp()
    $('.twentylink').slideDown()
    $('.headerlink').slideUp()

    setTimeout(() => {
      $('.headermain').removeClass('headermain_hasbanner')
    },300)
  ...
  $(window).scroll(function () {
    if ($("#smzbanner") && $("#smzbanner").length) {
      if (
        $(this).scrollTop() >
        $("#smzbanner").offset().top + $("#smzbanner").height() - 110
      ) {
        smzBanner()
        $("#smzbanner img").css("opacity", 0)
                $("#smzbanner img").css("display", 'none')

      } else {
        smzBanner()
        $("#smzbanner img").css("opacity", 1)
                $("#smzbanner img").css("display", 'block')

      }
    }

    /*  if ($('#smzbanner') && $('#smzbanner').length) {
  if ($(this).scrollTop() > $('#smzbanner').offset().top &&
    $(this).scrollTop() < $('#smzbanner').offset().top + $('#smzbanner').height() + 40) {
    $('#smzbanner').addClass('fixed')
  } else {
    $('#smzbanner').removeClass('fixed')
  }
}  */
  })
  ...
  const lbimg = (item) => {
    item.wrap(`<a href="${item.attr('src')}" data-lightbox="detailimg"></a>`);
  }
  $('.pagedetail__content img').each((index, item)  => {
    lbimg($(item))
  })
  ...
  $(".search__button_clear").click(function (e) {
    e.preventDefault()
    $(this).closest('.search_wrapper').find("input").val('')
    
})

  $('[data-toggleheader]').on('click', function(e) {
    let block = $(this).data('toggleheader')
    $(`[data-togglecontent=${block}]`).toggleClass('active')
  ...
  $('[data-click="corp__bi"]').on("click", function (e) {
    e.preventDefault()
    $('[data-toggle="corp__bi"]').toggle()
    $('[data-place="corp__bi"]').toggle()
  })
  if ($(window).width() <= 999) {
...
/*   $(".search__button_close").click(function (e) {
  ...
  $(".s-overlay").click(function (e) {
    e.preventDefault()
    $(".search__full").toggleClass("search__full_open")
    $(".searchbutton").toggleClass("search__button_open")
    $('[data-entity="search-result"]').html('')
...
$(document).ready(function () {
  ...
  if ($('[data-entity="search-wrapper"]').length > 0) {
    let searchInput = document.getElementById('title-search-input_fixed');
    ...
    searchInput.addEventListener('keystopped', function (event) {
      let searchValue = $(searchInput).val(),
          $this = $(this),
          $autocomplete = $('[data-entity="search-result"]');

      if (searchValue.length > 2) {
        $.get('/local/ajax/search.php', {q: searchValue, ajax: 'Y'}, function (result) {
          $autocomplete.html(result);
          $('[data-entity="search-wrapper"]').addClass('active');
        });
      } else {
        $autocomplete.html('');
        $('[data-entity="search-wrapper"]').removeClass('active');
      }
    }, false);
  }
  if ($('[data-entity="search-wrapper-mobile"]').length > 0) {
    let searchInput = document.getElementById('title-search-input_fixed_mobile');
    searchInput.addEventListener('keydown', function (event) {
      clearTimeout(keystoppedTimer);
      keystoppedTimer = setTimeout(function () {
        event.target.dispatchEvent(new Event('keystopped'));
    ...
    searchInput.addEventListener('keystopped', function (event) {
      let searchValue = $(searchInput).val(),
          $this = $(this),
          $autocomplete = $('[data-entity="search-result-mobile"]');

      if (searchValue.length > 2) {
        $.get('/local/ajax/search.php', {q: searchValue, ajax: 'Y'}, function (result) {
          $autocomplete.html(result);
          $('[data-entity="search-wrapper-mobile"]').addClass('active');
        });
      } else {
        $autocomplete.html('');
        $('[data-entity="search-wrapper-mobile"]').removeClass('active');
      }
    }, false);
  }
})
